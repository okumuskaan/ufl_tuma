        ...
        def evaluate(num_clients, data_handler, idx_train, server):
            ...
            for i in range(num_clients):
                ...
                dataloader = DataLoader(datasubset,
                    ...
                    shuffle=True,
                    pin_memory=True)                
                loss_i, _ = server.eval(server.model, dataloader, device=server.device)
...
def simulate_ufl_for_GUI(
    ...
    for idx_FL_rnd in range(fl_config["num_rounds"]):
        print(f"FL_rnd: {idx_FL_rnd+1}/{fl_config['num_rounds']}")
        fl_bar.progress((idx_FL_rnd+1)/fl_config["num_rounds"], text=f"FL rounds in progress, round: {idx_FL_rnd+1}/{fl_config['num_rounds']}")
        ...
        if quant_config["apply"]:
            ...
            comm_manager.run_comm_for_GUI(comm_bar)
            rx_quant_indices, rx_mults, est_n_sel_clients = comm_manager.return_output()
            # Phase 5: Data receive at the server and model aggregation 
            server.receive_tuma_outputs(rx_quant_indices, rx_mults, est_n_sel_clients)
        else:
        ...
        # For GUI plots:
        df = return_df_from_csv(logger.out_fname)  
        display_round_performance_plots_func(df)
        display_round_number_of_selected_clients_func(df)

        if (idx_FL_rnd+1)==fl_config["num_rounds"]:
            fl_bar.progress((idx_FL_rnd+1)/fl_config["num_rounds"], text="FL completed.")
